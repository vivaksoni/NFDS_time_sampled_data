...
t_abc <- head(read.table(file.path("ABC", paste("readyForABC_", demog, "_0.txt", sep=''))
                         , h=T, sep='\t'), 3100)
t_par <- head(read.table("params/fp_0.txt",
                         sep='\t', col.names = c('param','Tb','Feq')), 3100)
#get time and make stats:
t_par <- cbind(t_par[,c(2:3)])
t_sim_stats <- cbind(t_abc[,c(2:121)])

t_emp_stats <- read.table(file.path("empirical_sims", paste(demog, "_ABC2.txt", sep=''))
                          , h=T)
...
write.table(m_adj_values, file.path("/Volumes/WD/nfds/posteriors/nnet", 
                                    paste(demog, "_0.txt", sep='')), 
            sep='\t', quote=FALSE, row.names=FALSE)

df <- data.frame(head(Tb_pos$breaks,-1), Tb_pos$density)
colnames(df) <- c("Tb", "density")
write.table(df, file.path("/Volumes/WD/nfds/posteriors/nnet", paste(demog, "_Tb_0.txt", sep='')), 
            sep='\t', quote=FALSE, row.names=FALSE)

df <- data.frame(head(Feq_pos$breaks,-1), Feq_pos$density)
colnames(df) <- c("Feq", "density")
write.table(df, file.path("/Volumes/WD/nfds/posteriors/nnet", paste(demog, "_Feq_0.txt", sep='')), 
            sep='\t', quote=FALSE, row.names=FALSE)
